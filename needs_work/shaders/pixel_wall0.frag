...
void main()
{
    ...
    if (fragCoord.x < 64.0 && fragCoord.y < 64.0)
    {
        ...
        float f = 3.0 * texture(iChannel0, vec2(l, 0.75)).g
            - texture(iChannel0, vec2(l - 1.0 / texRes0.x, 0.75)).g
            - texture(iChannel0, vec2(l + 1.0 / texRes0.x, 0.75)).g;

        fragColor.x = f;
    }

    vec4 last = texture(iChannel1, fragCoord / texRes1);

    float last_res_x = floor(texture(iChannel1, (vec2(0.5, 70.5) / texRes1)).x);
    float last_res_y = floor(texture(iChannel1, (vec2(1.5, 70.5) / texRes1)).x);
    ...
    float last_time = texture(iChannel1, (vec2(2.5, 70.5) / texRes1)).x;
    ...
    fragColor.y = texture(iChannel1,
        clamp(floor(xt) + ft + 0.5, 0.0, 63.5) / texRes1).x;
