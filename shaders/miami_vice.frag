...
// mcguirev10 - The original uses #define to vary the display; here we use the
// four random values in vec4 randomrun4 to enable or disable the settings, and
// the code has been significantly altered to make all of these combos available
// instead of just compiling one variant or another

// Try different approximations of "cubes" by changing between DF0-DF7
//#define DF2
int cube = int(floor(clamp(randomrun4.x * 8.0, 0.0, 7.0)));

// Some "cubes" can be more or less boxy, define or comment out
//#define BOXY 
bool boxy = (randomrun4.y >= 0.5) ? true : false;

// Some "cubes" have flair variants, define or comment out
//#define FLAIR
bool flair = (randomrun4.z >= 0.5) ? true : false;

// mcguirev10 - these were also originally defined for certain cube effect (DFx) values
bool backstep =
       (cube == 2 || cube == 3 || cube == 5 || cube == 6)
    || (cube == 0 && !boxy)
    || (cube == 1 && !boxy);
...
float synth(vec2 p, float aa, out float h, out float db) {
  ...
  for (int i = -around; i <=around ;++i) {
    float fft = texture(iChannel0, vec2((n+float(i))*st, 0.25)).g; 
    fft *= fft;
    if (i == 0) h = fft;
...
//    #endif
    ...
    float dfeffect0(vec3 p, out float ogd) {
        const float sz = 20.0;
        #if defined(BOXY)
...
//    #endif
    ...
    float dfeffect1(vec3 p, out float ogd) {
    const float sz = 20.0;
    #if defined(BOXY)
...
//    #define BOUNCE_ONCE
    ...
    float dfeffect2(vec3 p, out float ogd) {
    const float sz = (20.0);
    vec3 p0 = p;
    vec3 p1 = p;
    p1 *= (g_rot);
...
//    #define BOUNCE_ONCE
    ...
    float dfeffect3(vec3 p, out float ogd) {
    const float sz = (20.0);
    vec3 p0 = p;
    vec3 p1 = p;
    float d0 = sphere8(p0, 20.0);
...
//    #endif
    ...
    float dfeffect4(vec3 p, out float ogd) {
    const float z = 10.0;
    vec3 p0 = p/z;
    float d2;
    float d0 = mb(p0, d2);
...
//    #define BOUNCE_ONCE
    ...
    float dfeffect5(vec3 p, out float ogd) {
    const float sz = (22.0);
    float d = 1E3;
    ogd = 1E3;
    mat3 rot = g_rot;
...
//    #define BOUNCE_ONCE
    ...
    float dfeffect6(vec3 p, out float ogd) {
    vec3 p0 = p;
...
//#else
    ...
    float dfeffect7(vec3 p, out float ogd) {
    const float sz = (20.0);
    vec3 p0 = p;
...
float df(vec3 p) {
  ...
  // mcguirev10 - what a pain in the ass
  float d1 = 0.0;
  switch(cube) {
    case 0: d1 = dfeffect0(p, gd1); break;
    case 1: d1 = dfeffect1(p, gd1); break;
    case 2: d1 = dfeffect2(p, gd1); break;
    case 3: d1 = dfeffect3(p, gd1); break;
    case 4: d1 = dfeffect4(p, gd1); break;
    case 5: d1 = dfeffect5(p, gd1); break;
    case 6: d1 = dfeffect6(p, gd1); break;
    case 7: d1 = dfeffect7(p, gd1); break;
  };
...
float rayMarchHi(vec3 ro, vec3 rd, float tinit, out int iter) {
  ...
  for (i = 0; i < MAX_RAY_MARCHES_HI; ++i) {
    float d = df(ro + rd*t);
    //#if defined(BACKSTEP)
      if (backstep && d<dti.x) { dti=vec2(d,t); }
    //#endif  
    if (d < TOLERANCE || t > MAX_RAY_LENGTH) {
      break;
    }
  ...
  //#if defined(BACKSTEP)
    if(backstep && i==MAX_RAY_MARCHES_HI) { t=dti.y; };
  //#endif  
  iter = i;
  return t;
}
