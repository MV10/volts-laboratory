...
#define iChannel3 eyecandyShadertoy
#define iTime time
...
float Map(vec3 pos)
{
    ...
    float fft = textureLod( iChannel3, vec2(0.5,0.15), 0.0).g;
    float angle = GetAngle(pos);
    float r = sin(pos.z*.1)*0.5 + 3. + Map5(pos)*.3 + fft * 2.0 + sin(angle*ARMS)*.3;
    //r += texture( iChannel3, vec2(fract(pos.z*.1),0.0) ).g;
...
void main()
{
    ...
    float fft = texture( iChannel3, vec2(0.2,0.25) ).g;
    fft = max(.0, fft - .5);
