...
float obj(in vec3 p)
{
  ...
  uv/=16.0;
  vec3 p3=p;
  p3.z-=(textureLod(iChannel0,uv,0.0).g-0.2)*4.0;	
  float c1=sdBox(p3,vec3(0.2,0.2,1.0));
  p.z=p.z-12.0;
  uv.y=0.9;
  p.xy=rot(p.xy,textureLod(iChannel0,uv,0.0).g*2.0*PI);
  uv.y=0.1;	
  p.yz=rot(p.yz,textureLod(iChannel0,uv,0.0).g*2.0*PI);	
...
vec3 camera(
  ...
  in vec2 coord)
{
  vec2 vPos=-1.0+2.0*coord.xy/iResolution.xy;
