...
float map(vec3 p)
{
    p = rotx(p,expOut(sin(xtime*1.8),1.1));
    p = roty(p,-expOut(sin(-xtime*1.5),1.));
...
{
    float mp = map(p);
   	p = mix(p, p/(-(mp+1.4)),.4+(sin(xtime*2.+sin(xtime))*0.15));
...
{   
    ...
    xtime = (iTime - fadeIn*5. + (mTex.w-0.05)*.7);
  
    //float w = textureLod(iChannel3, vec2(0.9,0.5), 0.).x;
    
    vec3 bgcol = mix(vec3(.1,1,.8),vec3(.1,.8,1.),noise(rd*2.+xtime*.7));
    ...
    col *= sin(gl_FragCoord.y*350.+xtime)*0.09+1.;//Scanlines
    col *= sin(gl_FragCoord.x*350.+xtime)*0.09+1.;
