...
void main() {
  ...
  float multiplyHistory =   8.0;
  float multiplyRealtime = 10.0;

  float su = hash(pointV * 13.7);
  //float snd = texture(sound, vec2(mix(0.001, 0.115, su), circleV * 0.5)).g;
    float snd = texture(sound, vec2(mix(0.003, 0.300, su), circleV * 0.5)).g * multiplyHistory;
    
  float q = (odd + quad * 2.) / 3.;
  //float sq = texture(sound, vec2(mix(0.001, 0.115, 0.), 0)).g;
    float sq = texture(sound, vec2(mix(0.002, 0.110, 0.), 0)).g * multiplyRealtime;
