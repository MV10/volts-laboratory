...
void main() {
	...
	float freqs[6];
	//Sound
	freqs[0] = texture( iChannel0, vec2( 0.01, 0.25 ) ).g;
	freqs[1] = texture( iChannel0, vec2( 0.03, 0.25 ) ).g;
	freqs[2] = texture( iChannel0, vec2( 0.05, 0.25 ) ).g;
	freqs[3] = texture( iChannel0, vec2( 0.07, 0.25 ) ).g;
	freqs[4] = texture( iChannel0, vec2( 0.17, 0.25 ) ).g;
	freqs[5] = texture( iChannel0, vec2( 0.25, 0.25 ) ).g;
