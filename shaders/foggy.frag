...
// Requires a very high-spec machine
//#define HIGH_QUALITY
...
void main() 
{
    ...
    {
        bass += texelFetch( iChannel0, ivec2(range,0), 0 ).g;
    }
    ...
    for (float i = 1.0; i <= LAYERS_COUNT ; i++)
    {
        ...
        col += fancyLayer(planeUV.xy, 1.2 - (0.3 * bass) - pow(i / LAYERS_COUNT, 2.0)) * COLOR_MULTIPLIER * pow(0.99, i);
