...
// Lots of unnecessary garbage in this one cut-and-pasted from the
// guy's older shaders, but good luck untangling it. Still looks cool.
...
vec3 camPath(in float _time) {
    
    vec2 mouse = iMouse.xy / iResolution.xy;
    mouse = mouse * kPI * 2.0 + kPI;
    
    vec3 camPos = vec3(sin(_time * 0.3) * 2.0, sin(_time * 0.13) * 2.0 + 4.5, _time);
	
    //R(camPos.xz, _time + mouse.x);
    //R(camPos.zy, sin(_time) * kPI * 0.1 + 0.2);
