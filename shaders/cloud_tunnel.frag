...
{	
    ...
    bsMo = vec2(0.5); // no mouse: (iMouse.xy - 0.5*iResolution.xy)/iResolution.y;
    
    float time3x = time*3.;
    vec3 ro = vec3(0,0,time3x);
    ...
    vec3 target = normalize(ro - vec3(disp(time3x + tgtDst)*dspAmp, time3x + tgtDst));
    ro.x -= bsMo.x*2.;
    vec3 rightdir = normalize(cross(target, vec3(0,1,0)));
    vec3 updir = normalize(cross(rightdir, target));
    rightdir = normalize(cross(updir, target));
	vec3 rd=normalize((p.x*rightdir + p.y*updir)*1. - target);
    rd.xy *= rot(-disp(time3x + 3.5).x*0.2 + bsMo.x);
    prm1 = smoothstep(-0.4, 0.4,sin(time*0.3));
	vec4 scn = render(ro, rd, time3x);
