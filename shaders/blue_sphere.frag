...
float sphereSound( in vec3 checkedPoint )
{
    ...
    return ( texture( iChannel0, samplerSmall ).g * texture( iChannel0, samplerBig ).g ) * 2.5;
}
...
{    
    float sound = texture( iChannel0, vec2( cos( fragData.normalDistance ), 0.25 )).g 
                + texture( iChannel0, vec2( sin( fragData.normalDistance ), 0.25 )).g;
...
void main()
{
    ...
    const float camSurfaceRadius = 3.0;
