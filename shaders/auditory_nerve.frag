...
float noise(vec3 p){
    ...
    float a = texture(iChannel0,t/20.+np.xy).g + texture(iChannel1,t/20.+np.xy).y;
    float b = texture(iChannel0,t/20.+.77+np.yz).g + texture(iChannel1,t/20.+.77+np.yz).y;
...
void main()
{
    ...
    for (int r = 1; r <= rayCount; r++)
    {
        ...
        // Rotation 
       	...
       	p = rotateY(p, 0.5/iResolution.x * 2.* 3.14);  
        p = rotateY(p,iTime/3.);
